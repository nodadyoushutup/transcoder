"""Logging helpers for the webserver application."""
...
def configure_logging(prefix: str, *, log_dir: Optional[Path] = None) -> Path:
    """Configure logging to write under the webserver backend directory."""
    ...
    env_dir = os.getenv("WEBSERVER_LOG_DIR")
    log_directory = Path(env_dir).expanduser() if env_dir else backend_root / "logs"
