  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col bg-transparent">
      <header className="flex items-center justify-between border-b border-border/80 px-6 py-4">
        <h2 className="text-lg font-semibold text-foreground">Viewers</h2>
      </header>
      <div className="flex-1 space-y-6 overflow-y-auto px-6 py-6">
        ...
        <section className="panel-section">
          ...
          {!error && !pending && activeUsers.length === 0 ? (
            <p className="mt-3 text-xs text-subtle">No authenticated viewers right now.</p>
          ) : null}
            ...
            {activeUsers.map((userItem) => {
              ...
              return (
                ...
                >
                  ...
                  {isSelf ? <span className="text-[10px] uppercase tracking-wide text-success">You</span> : null}
                </li>
              );
            })}
          </ul>
